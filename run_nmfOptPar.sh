...
# Set up file headers
echo "program,data_set,n,i,t,blk_size,num_threads,iter_0,iter_1,iter_2,iter_3,iter_4,iter_5,iter_6" >> $RESULT_FILE
...
for (( n=2 ; n<length ; n+=5 )); do
  ...
  # for p in {2..6}; do
  printf "$PROGRAM1,$data_set,$N,$I,$T,$B,1" >> $RESULT_FILE
  ...
  if [ $ITER1 == 0 ]; then
    while [ $ITER1 -lt 7 ]; do
      ...
      echo "ITER: $ITER1 for $PROGRAM1 for data_set=$data_set, n=$N, i=$I, t=$T, blk_size=$B"
...
#   B=1
  ...
  for i in {1..8}; do
    ...
    printf "$PROGRAM2,$data_set,$N,$I,$T,$B,$i" >> $RESULT_FILE
    ...
    while [ $ITER -lt 7 ]; do
      ...
      echo "ITER: $ITER for $PROGRAM2 for data_set=$data_set, n=$N, i=$I, t=$T, blk_size=$B, num_threads=$i"
