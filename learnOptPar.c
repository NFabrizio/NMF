/*
 * learnOptPar.c - original file copied from learnOpt.c
 */
#include <stdio.h>
#include <stdlib.h>
#include <time.h>
...
#pragma omp parallel for private (i, j, k)
    ...
    for(it = 0;it < maxiter;it++){
        ...
        isd = 0.0;
#pragma omp parallel for private (i, j) reduction(+:isd)
        for (ii = 0; ii < n_rows; ii+=blk_size) {
          for(i = ii; i < min(ii+blk_size, n_rows); i++){
              for(j = 0;j < n_cols;j++){
...
#pragma omp parallel for private (i, j, k, jj, numerator, denominator)
        for (ii = 0; ii < n_rows; ii+=blk_size) {
          for(i = ii; i < min(ii+blk_size, n_rows); i++){
              for(k = 0;k < n_class;k++){
                  if(W[i][k] != 0.0){
...
#pragma omp parallel for private (i, j, k)
        ...
        for (k = 0; k < n_class; k++) {
#pragma omp parallel for private (i, j, ii, numerator, denominator)
            for (jj = 0; jj < n_cols; jj += blk_size) {
                for (j = jj; j < min(jj + blk_size, n_cols); j++) {
                    if (H[k][j] != 0.0) {
                        numerator = 0.0;
